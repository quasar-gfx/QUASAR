...
int main(int argc, char** argv) {
    ...
    guiManager->onRender([&](double now, double dt) {
        ...
        if (showUI) {
            ...
            float depthOffsetSizeMB = static_cast<float>(multiSimulator.stats.totalDepthOffsets * sizeof(unsigned int16_t)) / BYTES_IN_MB;
        ...
        if (showCaptureWindow) {
            ...
            if (ImGui::Button("Capture Current Frame")) {
                ...
                for (int view = 1; view < maxViews; view++) {
                    ...
                    if (saveAsHDR) {
                        multiSimulator.serverFrameRTs[view].saveColorAsHDR(fileName + ".hdr");
                    }
                    else {
    ...
    app.onRender([&](double now, double dt) {
        ...
        if (generateRemoteFrame) {
            ...
            spdlog::info("  Fill Quads Time: {:.3f}ms", multiSimulator.stats.totalFillQuadsTime);
            spdlog::info("Create Mesh Time: {:.3f}ms", multiSimulator.stats.totalCreateMeshTime);
            spdlog::info("  Append Quads Time: {:.3f}ms", multiSimulator.stats.totalAppendProxiesTime);
