...
struct FPlane {
	...
	FPlane() = default;
	FPlane(const glm::vec3& p1, const glm::vec3& norm) : normal(glm::normalize(norm)), distance(glm::dot(normal, p1)) {}
    FPlane(const glm::vec3& norm, float dist) : normal(glm::normalize(norm)), distance(dist) {}
...
public:
    ...
    void update(glm::vec3 position, glm::vec3 front, glm::vec3 right, glm::vec3 up, float zNear, float zFar, float aspect, float fovY) {
