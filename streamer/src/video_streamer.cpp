...
int VideoStreamer::sendFrame() {
    ...
    //Get Texture
    checkCudaErrors(cudaGraphicsResourceSetMapFlags(cuInpTexRes, CU_GRAPHICS_MAP_RESOURCE_FLAGS_READ_ONLY));
    checkCudaErrors(cudaGraphicsMapResources(1, &cuInpTexRes, 0));

    //Map texture to cuda array
    cudaGraphicsSubResourceGetMappedArray(&mappedArray, cuInpTexRes, 0, 0); // Nvidia says its good practice to remap each iteration as OGL can move things around

    //Release texture
    cudaGraphicsUnmapResources(1, &cuInpTexRes, 0);
